...
func _append(file *os.File, contents string) {
	_, err := file.WriteString(contents)
	check(err)
	file.Sync()
}
...
func get_for_each_runtime_cost(i int, size int) int64 {

	empty := get_empty_runtime_cost(1000)
	...
	if size == 0 {
		...
		return time.Since(start2).Nanoseconds() - empty 
	}
	...
	elapsed1 := (time.Since(start1).Nanoseconds() - empty)  * int64(size)
	...
	elapsed2 := time.Since(start2).Nanoseconds() - empty 
...
func run_experiment(experiment func(int) int64, experiment_name string, subtraction float64) float64 {
	...
	time_taken := math.Max(float64(experiment(1000))-subtraction, 0.)
...
func run_experiment_scaled(experiment func(int, int) int64, experiment_name string, num_sizes int, subtraction_init float64, subtraction_iter float64) []float64 {
	...
	for index, size := range size_seq {
		time_taken := math.Max(float64(experiment(1000, size))-subtraction_init-subtraction_iter*float64(size), 0.)
...
func float_to_str_list(list []float64) []string { 
	var str_list []string 

	for _, item := range list { 
		str_list = append(str_list, fmt.Sprintf("%f", item)) 
	}
...
func main() {
	...
	csv_writer.Write([]string{fmt.Sprintf("%f", empty), fmt.Sprintf("%f", assign), fmt.Sprintf("%f", var_read), fmt.Sprintf("%f", add), fmt.Sprintf("%f", mult), fmt.Sprintf("%f", concat), fmt.Sprintf("%f", bool_cond), fmt.Sprintf("%f", increment), fmt.Sprintf("%f", _if), fmt.Sprintf("%f", _open), fmt.Sprintf("%f", func_call), fmt.Sprintf("%f", for_loop)})
	...
	for_each := run_experiment_scaled(get_for_each_runtime_cost, "for each", num_sizes, 0., 0.) 

	csv_writer.Write(float_to_str_list(for_each))

	// fmt.Println("get print...")

	print := run_experiment_scaled(get_print_runtime_cost, "print", num_sizes, empty, 0.)
	csv_writer.Write(float_to_str_list(print))

	// fmt.Println("get read...")

	read := run_experiment_scaled(get_read_runtime_cost, "read", num_sizes, empty+assign+var_read, 0.)
	csv_writer.Write(float_to_str_list(read))

	// fmt.Println("get write...")

	_write := run_experiment_scaled(get_write_runtime_cost, "write", num_sizes, empty+2*var_read, 0.)
	csv_writer.Write(float_to_str_list(_write))


	// fmt.Println("get append...")

	_append := run_experiment_scaled(get_append_runtime_cost, "append", num_sizes, empty+2*var_read, 0.)
	csv_writer.Write(float_to_str_list(_append))
