...
let map_reduce delim f x = String.concat delim (List.map f x)
...
let string_of_user_func (user_func: user_func) = Fmt.str "%s(%s)" (string_of_id user_func.name) (map_reduce ", " string_of_expr user_func.args)

let string_of_write_template write_template = Fmt.str "%s, %s" (string_of_id write_template.file) (string_of_expr write_template.contents)

let string_of_func_call = function 
| User_func(user_func) -> string_of_user_func user_func
| Print(expr) -> Fmt.str "print(%s)" (string_of_expr expr)
| Input -> "input()"
  | Open(expr) -> Fmt.str "open(%s)" (string_of_expr expr)
    | Read(expr) -> Fmt.str "read(%s)" (string_of_expr expr)
      | Write(write_template) -> Fmt.str "write(%s)" (string_of_write_template write_template)
        | Append(write_template) ->  Fmt.str "append(%s)" (string_of_write_template write_template)
...
and string_of_structure = function
  ...
  | Block block -> string_of_block block
...
and string_of_block_record block_record =
  map_reduce "\n" string_of_command block_record.contents


and string_of_block = function
  | Block block_record -> string_of_block_record block_record
  | Ignore contents -> map_reduce "\n" string_of_command contents
  | Force contents -> map_reduce "\n" string_of_command contents

and string_of_param (id, type_id) = Fmt.str "%s %s" (string_of_id id) (string_of_type_id type_id)  

and string_of_func func =
  Fmt.str "func %s (%s) {\n%s\n}"
    (string_of_id func.name)
    (map_reduce ", " string_of_param func.params)
    (string_of_block func.body)


let string_of_program program =
  Fmt.str "package %s\n\n" (string_of_id program.package)
  ^ map_reduce "\n" string_of_var program.global_vars
  ^ "\n\n"
  ^ map_reduce "\n" string_of_func program.funcs
